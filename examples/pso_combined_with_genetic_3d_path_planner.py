...
from pyalgorithmlab.algorithm.pso_combined_with_genetic import (
    EnhancedGeneticAlgorithm,
    EnhancedParticleSwarmOptimizer,
...
if __name__ == "__main__":
    pso_args = PSOAlgorithmArguments(
        num_particles=100,
        num_dimensions=3,
        max_iterations=100,
    ...
    pso_optimizer = EnhancedParticleSwarmOptimizer(
        args=pso_args,
        problem_type=ProblemType.MIN,
        objective_function=path_planner.calculate_best_path_costs,
    )

    genetic_arguments = GeneticAlgorithmArguments(
        population_size=int(pso_args.num_particles / 2),  # 遗传算法只处理劣势粒子，所以是总粒子数的一半
        num_dimensions=pso_args.num_dimensions,
        tournament_size=3,
        max_generations=pso_args.max_iterations,
        individual_available_boundaries_min=pso_args.position_boundaries_min,
        individual_available_boundaries_max=pso_args.position_boundaries_max,
        crossover_rate=0.5,
        mutation_rate=0.5,
    )

    genetic_algorithm = EnhancedGeneticAlgorithm(args=genetic_arguments, problem_type=ProblemType.MIN)

    best_fitness_values = pso_optimizer.start_iterating(genetic_algorithm)

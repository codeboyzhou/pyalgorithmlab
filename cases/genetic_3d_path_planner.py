...
class PathPlanner3D:
    ...
    def __init__(
        ...
        # 初始化XY平面坐标
        x = np.linspace(
            algorithm_args.individual_available_boundaries_min[0],
            algorithm_args.individual_available_boundaries_max[0],
            100,
        )
        y = np.linspace(
            algorithm_args.individual_available_boundaries_min[1],
            algorithm_args.individual_available_boundaries_max[1],
            100,
        )
    ...
    def calculate_best_path_costs(self, individuals: np.ndarray) -> np.ndarray:
        """
        遗传算法目标函数：计算个体路径成本，规划最优路径点

        Args:
            individuals: 个体位置数组，形状为 (num_individuals, num_dimensions)

        Returns:
            个体路径成本数组，形状为 (num_individuals,)
        """
        # 初始化个体路径成本
        num_individuals = individuals.shape[0]
        costs = np.zeros(num_individuals)

        # 当前进化代数
        generation = len(self.best_path_points) - 1
        generation_progress = generation / self.algorithm_args.max_generations

        # 定义各项惩罚权重
        distance_to_destination_penalty_weight = 0.1 + 0.3 * generation_progress
        distance_to_shortest_line_penalty_weight = 0.4 - 0.1 * (generation_progress**3)
        path_direction_angle_penalty_weight = 0.5 + 0.5 * generation_progress
        point_gathering_penalty_weight = (0.3 + 0.4 * generation_progress) * (0.5 if generation_progress < 0.3 else 1.0)
        terrain_collision_penalty_weight = 0.7 + 0.3 * generation_progress

        ### 到终点的距离惩罚，鼓励路径点向终点靠拢
        destination = self.destination.to_ndarray()
        distance_to_destination = np.linalg.norm(individuals - destination, axis=1)
        ...
        distance_to_line = ndarrays.point_to_line_distance(individuals, start_point, destination)
        ...
        if len(self.best_path_points) >= 3:
            ...
            angles = np.array([ndarrays.angle_degrees(point1, point2, point3) for point3 in individuals])
        ...
        point_gathering_costs = np.linalg.norm(individuals - previous_point.to_ndarray(), axis=1)
        ...
        terrain_proximity_costs = np.array(
            [self.terrain.min_horizontal_distance_to_peak_centers(Point.from_ndarray(p)) for p in individuals]
        )
        ...
        # 选择成本最优且满足约束条件的点
        candidate_point = self.choose_candidate_point(costs, individuals, previous_point)
        if candidate_point is not None:
            self.best_path_points.append(candidate_point)

        return costs

    def choose_candidate_point(self, costs: np.ndarray, individuals: np.ndarray, previous_point: Point) -> Point | None:
        """
        选择成本最优且满足约束条件的路径点

        Args:
            costs: 当前路径总成本数组，形状为 (num_individuals,)
            individuals: 遗传算法个体数组，形状为 (num_individuals, num_dimensions)
        ...
        for index in sorted_indices:
            candidate_point = Point.from_ndarray(individuals[index])
...
if __name__ == "__main__":
    genetic_arguments = AlgorithmArguments(
        ...
        tournament_size=3,
        max_generations=300,
        individual_available_boundaries_min=(0, 0, 1),
        individual_available_boundaries_max=(100, 100, 5),
        crossover_rate=0.8,
        mutation_rate=0.2,
    )

    path_planner = PathPlanner3D(
        algorithm_args=genetic_arguments,
    ...
    best_fitness_values = genetic_algorithm.start_iterating(objective_function=path_planner.calculate_best_path_costs)
